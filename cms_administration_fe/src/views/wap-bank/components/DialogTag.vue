<template>
  <el-dialog :title="dialog.titleName" :visible.sync="showDialog" width="40%" :before-close="handleClose">
    <el-form ref="ruleForm" v-loading="dialogLoading" :rules="rules" :model="objectData" class="demo-ruleForm">
      <el-row>
        <el-col :sm="24" :md="24">
          <el-form-item label="Tên tag :" required>
...
export default {
  ...
  data() {
    return {
      ...
      dialogLoading: false,
      formRequest: {},
      dialog: { titleName: 'Thêm mới tag', buttonName: 'Thêm mới' }
    }
  },
  watch: {
    objectData(value) {
    ...
    isAdd(value) {
      ...
      if (value) {
        this.dialog = Object.assign({}, { titleName: 'Thêm mới tag', buttonName: 'Thêm mới' })
      } else {
        this.dialog = Object.assign({}, { titleName: 'Cập nhật tag', buttonName: 'Cập nhật' })
      }
    }
  },
  methods: {
    ...
    async handleSubmitAdd() {
      wapBankResource.store_tag(this.formRequest).then(res => {
        if (res.error_code === 0) {
    ...
    async handleSubmitUpdate() {
      wapBankResource.update_tag(this.formRequest, this.formRequest.id).then(res => {
        if (res.error_code === 0) {
          this.$message.success(res.data)
          this.handleClose()
        } else {
