<template>
  <el-row style="margin:20px; font-family:Sans-serif">
    <el-row>
      <el-col :sm="24">
        <h3 style="margin: 0; width: 100%; padding: 10px; text-align:center;">DANH SÁCH TAG </h3>
      </el-col>
    </el-row>
...
import DialogTag from '@/views/wap-bank/components/DialogTag.vue'
...
export default {
  ...
  methods: {
    async onGetList() {
      ...
      wapBankResource.list_tag(this.query).then(res => {
    ...
    handleDelete(row) {
      this.$confirm(`Bạn có chắc chắn muốn xóa tag ${row.name}`).then(async() => {
        wapBankResource.delete_tag({ id: row.id }).then(res => {
          const { error_code, data, error_msg } = res
          if (error_code === 1) {
            this.$message.error(error_msg)
            return
