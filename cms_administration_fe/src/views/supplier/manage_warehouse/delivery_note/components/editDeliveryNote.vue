<template>
  <el-dialog
    ...
    width="90%"
  >
    <editTableNote v-if="showEditTableNote" :type="typeHandleDialog" :dialog-form-visible="showEditTableNote" :data-body-old="dataEditRow" :warehouse-id="id" @on-close="showEditTableNote=false" @on-add="addRowNote" @on-edit="editRowNote" />
    <el-form ref="ruleForm" :rules="rules" style="margin-bottom: 40px" :model="objData">
      <h5 style="color: black;">THÔNG TIN PHIẾU XUẤT KHO</h5>
    ...
    <div>
      ...
      >
        ...
        <el-table-column :label="('Hành động')" align="center" width="130px">
          <template slot-scope="scope">
            <el-button type="primary" icon="el-icon-edit" circle @click="onEditRow(scope.row)" />
...
<script>
import Vue from 'vue'
import editTableNote from './editTableNote'
  ...
  methods: {
    ...
    editDeliveryNote() {
      warehouseApi.edit_warehouse_note(this.dataUpdate).then(res => {
        ...
        console.log('..........editDeliveryNote ', JSON.stringify(data))
        if (error === 0) {
        ...
        console.log('..........editDeliveryNote ', err)
        this.$message.error(err.message)
      })
      console.log('editDeliveryNote')
    },
    listAllShop() {
