<template>
  <div id="received-note">
    <addDeliveryNote v-if="showDialogAddDeliveryNote" :show-detail="showDialogAddDeliveryNote" @close-dialog="showDialogAddDeliveryNote=false" @success-create-note="resetFilter" />
    <editDeliveryNote v-if="showDialogEditDeliveryNote" :id="edit_id" :show-detail="showDialogEditDeliveryNote" @close-dialog="showDialogEditDeliveryNote=false" @success-edit-note="handleSuccessEdit" />
    <viewDeliveryNote v-if="showDialogViewDeliveryNote" :id="view_id" :show-detail="showDialogViewDeliveryNote" @close-dialog="showDialogViewDeliveryNote=false" />
    <div style="width: 100; display: flex; justify-content: space-between;">
      <div style="display: flex; align-items: center;">
        <p style="margin-right: 15px; margin-bottom: 0px" @click="onBack"><i class="el-icon-back" /></p>
        <h3 style="margin: 0px">Quản lý phiếu xuất kho</h3>
      </div>
      <div style="display: flex; justify-content: end; height: 40px">
        <el-button v-if="is_write" type="primary" icon="el-icon-plus" @click="onAddReceivedNote">Tạo phiếu xuất kho</el-button>
    ...
    <div style="display: flex; justify-content: space-between; flex-wrap: wrap; margin-top: 10px">
      <div>
        ...
        <el-button style="margin-left: 10px" type="primary" icon="el-icon-search" @click="onSearchDeliveryNote">Tìm kiếm</el-button>
      </div>
    ...
    <div id="brand-table-id" style="margin-top: 10px;">
      <el-tabs v-model="tabName" @tab-click="handleChangeTab">
        <el-tab-pane label="Danh sách phiếu xuất" name="first" />
        <el-tab-pane label="Phiếu xuất đã hủy" name="second" />
      </el-tabs>
      <el-table v-loading="loading" :data="tableData" align="center" border style="width: 100%">
        <el-table-column prop="id" label="Mã phiếu xuất" align="center">
          <template slot-scope="scope">
            <el-link :underline="false" style="color:#4CA1D5" @click="handleView(scope.row)">XK{{ scope.row.id }}</el-link>
          </template>
        </el-table-column>
        <el-table-column prop="name" label="Tên phiếu xuất" align="center" />
        <el-table-column prop="delivery_id" label="ID Bill" align="center">
          <template slot-scope="scope">
            {{ scope.row.delivery_id ? `DL${scope.row.delivery_id}` : '' }}
          </template>
        </el-table-column>
        ...
        <el-table-column prop="export_time" label="Ngày xuất kho" align="center">
          <template slot-scope="scope">
            {{ scope.row.export_time ? new Date(scope.row.export_time).toISOString().slice(0, 10) : handleImportTime(scope.row.import_time) }}
          </template>
        </el-table-column>

        <el-table-column prop="user.username" label="Người xuất" align="center">
          <template slot-scope="scope">
            <span v-if="scope.row.type_system===1">Hệ thống</span>
...
import addDeliveryNote from './components/addDeliveryNote'
import WarehouseResource from '@/api/manage_supplier/warehouse'
import editDeliveryNote from './components/editDeliveryNote'
...
export default {
  components: {
    Pagination, addDeliveryNote, editDeliveryNote, viewDeliveryNote
  },
  data() {
    return {
      dateRange: '',
      ...
      filterSearch: 2,
      searchOption: [
        { id: 2, name: 'Tìm theo tên phiếu xuất' },
        { id: 1, name: 'Tìm theo mã phiếu xuất' }
      ],
      loading: false,
      tableData: [],
      total: 0,
      showDialogAddDeliveryNote: false,
      showDialogEditDeliveryNote: false,
      showDialogViewDeliveryNote: false,
      edit_id: null,
      view_id: null,
      tabName: 'first'
    }
  ...
  methods: {
    ...
    handleEdit(row) {
      this.showDialogEditDeliveryNote = true
      this.edit_id = row.id
    },
    handleView(row) {
      this.showDialogViewDeliveryNote = true
      this.view_id = row.id
    },
    deleteWarehouseNote(data) {
      ...
      warehouseApi.delete_warehouse_note(data).then(res => {
        const { error } = res
        if (error !== 0) throw new Error('Lỗi xóa phiếu xuất')
        this.$message.success('Xóa phiếu xuất thành công')
        this.loading = false
        this.getTableData()
      }).catch(err => {
        console.log(err)
    ...
    handleDelete(row) {
      this.$confirm('Bạn có chắc muốn xóa phiếu xuất này?', 'Warning', {
    ...
    onAddReceivedNote() {
      this.showDialogAddDeliveryNote = true
    },
