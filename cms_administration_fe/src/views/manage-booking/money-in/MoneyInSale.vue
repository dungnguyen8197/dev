<template>
  <el-row style="margin:20px;">
    <el-row>
      <el-col :sm="24">
        <h3 style="margin: 0; width: 100%; padding: 10px; text-align:center;">{{ "MÀN CỦA SALE" }}</h3>
      </el-col>
    </el-row>
    <div class="clearfix-20" />
    <el-row>
      <el-col :sm="24">
        <h4 style="margin: 0; width: 100%; padding: 10px; color: red;text-align:center;">{{ `Tổng số tiền: ${formatNumber(total_money)} VNĐ` }}</h4>
      </el-col>
    </el-row>
    ...
    <el-row style="margin-top: 40px;">
      ...
      >
        ...
        >
          <template slot-scope="scope">
            <template v-if="scope.row.sale">
              <el-tag>{{ scope.row.sale.name }}</el-tag>
              <el-button
                v-if="(checkAdmin(scope.row)) && scope.row.is_done === 0"
                :loading="loadingUnProcess"
                style="margin-top: 5px;"
                size="small"
                type="primary"
                @click="unConfirmProcess(scope.row)"
              >Nhả xử lý
              </el-button>
            </template>
            <el-button
              v-if="scope.row.sale == null && scope.row.is_clear === 0"
              style="margin-top: 5px;"
              size="small"
              type="primary"
              @click="confirmProcess(scope.row)"
            >Nhận xử lý
            </el-button>
            <template v-else>
              <el-tag>Hệ thống</el-tag>
            </template>
          </template>
        </el-table-column>
        <el-table-column
        ...
        >
          ...
          >
            ...
            <section>
              <el-tag v-if="scope.row.is_done == 1" type="success">
                Đã xác nhận
              </el-tag>
              <el-button v-else type="primary" size="small" @click="confirmDone(scope.row)">Xác nhận
              </el-button>
            </section>
          </section>
        </el-table-column>
      ...
      <el-row class="list-device__footer">
        ...
        />
        <!-- <dialog-edit-note
          :show-dialog="dialogUpdateNote"
          :object-data="objectData"
          @onCloseDialog="handleClose"
        /> -->
        <dialog-edit-history :show-dialog="showDialogUpdateRow" :is-add="isAdd" :object-data="objectDataEdit" @onCloseDialog="handleClose" />
        ...
        <el-dialog
          title="Xác nhận xong "
          :visible.sync="dialogConfirmDone"
          width="30%"
          center
        >
        ...
        <el-dialog
          title="Xác nhận clear or no "
          :visible.sync="dialogConfirmClear"
          width="30%"
          center
        >
          <span>Cập nhật TT clear ( No or Clear ) ?</span>
          <span slot="footer" class="dialog-footer">
...
import { LIST_BANK_ACCOUNT } from '@/utils/constant'
...
export default {
  ...
  methods: {
    checkAdmin(row) {
      if (row?.sale?.id === getAcountInfo()?.id) {
        return true
      }
      return false
    },
    ...
    handleEdit(row) {
      ...
      this.objectDataEdit = Object.assign({}, { ...row, is_sale: true })
    },
    handleAdd() {
      this.isAdd = true
      this.showDialogUpdateRow = true
    ...
    handleConfirmDone() {
      ...
      const body = {
        is_done: 1,
        time_done: this.request
      }
      moneyInResource.confirmDone(body, this.query.id).then(res => {
    ...
    confirmClear(row) {
      ...
      this.query.id = row.id
      this.body.is_clear = row.is_clear
    },
    ...
    unConfirmProcess(row) {
      this.loadingUnProcess = true
      moneyInResource.confirmUnProcess(row.id).then(res => {
        const { error_code, error_msg } = res
        this.loadingUnProcess = false
        if (error_code === 0) {
          this.$message.success(error_msg)
          this.onGetList()
        } else {
