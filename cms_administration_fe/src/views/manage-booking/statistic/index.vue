  ...
  <el-row>
    <el-row class="app-container list-account">
      ...
      <el-col :sm="24" style="display:flex;">
        <h3 style="margin: 0; width: 100%; padding: 10px; text-align: center;">Quản lý thu</h3>
      </el-col>
      <!-- </el-row> -->
        ...
        <el-row :gutter="20" style="margin-top: 10px;">
          <el-col :span="24" align="right">
            <div class="grid-content bg-purple">
              ...
              <el-button size="small" type="primary" @click="downloadSampleFile">Tải file mẫu</el-button>
            </div>
          </el-col>
        </el-row>
        <el-row>
      ...
      <el-row class="data-table-content table-responsive">
        <table v-loading="loadingTable" class="table table-bordered" style="font-size: 12px">
          <thead>
            <tr>
              <th scope="col" class="text-center vertical-middle">STT</th>
              <th scope="col" class="text-center vertical-middle">Pax id</th>
              <th scope="col" class="text-center vertical-middle">Sân</th>
              <th scope="col" class="text-center vertical-middle">Phụ phí(VND)</th>
              <th scope="col" class="text-center vertical-middle">Golfer</th>
              <th scope="col" class="text-center vertical-middle">Code booking</th>
              <th scope="col" class="text-center vertical-middle">Mã cuộc gọi</th>
              <th scope="col" class="text-center vertical-middle">Mã giảm giá</th>
              <th scope="col" class="text-center vertical-middle">Người đặt</th>
              <th scope="col" class="text-center vertical-middle">Nguồn</th>
              <th scope="col" class="text-center vertical-middle">Admin</th>
              <th scope="col" class="text-center vertical-middle">Người giới thiệu</th>
              <th scope="col" class="text-center vertical-middle">Giá</th>
              <th scope="col" class="text-center vertical-middle">Giá doanh thu</th>
              <th scope="col" class="text-center vertical-middle">Phụ phí cố định</th>
              <th scope="col" class="text-center vertical-middle">Giá gốc</th>
              <th scope="col" class="text-center vertical-middle">Refund</th>
              <th scope="col" class="text-center vertical-middle">Ngày đánh</th>
              <th scope="col" class="text-center vertical-middle">Ngày công nợ</th>
              ...
              <th scope="col" class="text-center vertical-middle">Ngày thanh toán</th>
              <th scope="col" class="text-center vertical-middle">Ngày đại lý thanh toán</th>
              <th scope="col" class="text-center vertical-middle" style="width: 200px">Note</th>
              <th scope="col" class="text-center vertical-middle" style="width: 200px">Thông tin khách xuất hoá đơn</th>
              <th scope="col" class="text-center vertical-middle">Code Payment</th>
              <th scope="col" class="text-center vertical-middle">CCCD / Passport</th>
              <th scope="col" class="text-center vertical-middle">Đối chiếu</th>
              <th scope="col" class="text-center vertical-middle">TT Xác nhận Booking</th>
              <th scope="col" class="text-center vertical-middle" />
            </tr>
          </thead>
          <tbody v-for="(value, i) in tableData" :key="i">
            ...
            >
              ...
              <td class="text-center">
                <template v-if="item['day_debt'] != null">
                  <div>
                    {{ convertDate(item['day_debt']) }}
                  </div>
                  <template v-if="checkDayDebt(item)">
                    <el-button size="mini" type="primary" @click="clickDebtCorrection(item)">Sửa công nợ</el-button>
                  </template>
                </template>
              </td>
              ...
              <td v-if="index === 0" class="text-center" :rowspan="value.booked.length">
                ...
                <div class="p-10">
                  <el-button icon="el-icon-upload" size="mini" type="primary" @click="exportVoucher(value)">Xuất chứng
                    từ
                  </el-button>
                </div>
              </td>
            ...
            <tr v-for="(billOutSide, keyBillOutSide) in value.billOutsidePayment" :key="keyBillOutSide + '-' + i">
              <td>{{ billOutSide.product_info.product_name }}</td>
              <td>{{ billOutSide.code_booking }}</td>
              <td />
              <td />
              <td>{{ billOutSide.user_id > 0 ? `VID` + billOutSide.user_id : '' }}</td>
              <td />
              <td />
              <td />
              <td>{{ formatNumberOrNull(billOutSide.real_payment) }}</td>
              <td>{{ formatNumberOrNull(billOutSide.real_payment) }}</td>
              <td />
              ...
              <td />
              <td>
                {{ convertDateTimeSecond(billOutSide['created_at']) }}
              </td>
              <td />
              <td class="text-center">
              ...
              <td>
                {{ billOutSide.date_payment != null ? convertDateTimeSecond(billOutSide.date_payment) : '' }}
              </td>
              <td />
              <td>
                {{ billOutSide.note }}
    ...
    />
    <dialog-import-excel :dialog-visible="showDialogImport" @onClickButtonImport="handleClickButtonImport" />
    <export-voucher
      :dialog-visible="showVoucher"
      :object-voucher="objectVoucher"
...
import DialogDebtCorrection from '@/views/manage-booking/statistic/components/DialogDebtCorrection.vue'
import exportVoucher from '@/views/manage-booking/statistic/components/exportVoucher.vue'
...
import DialogImportExcel from '@/views/manage-booking/statistic/components/DialogImportExcel.vue'
...
export default {
  name: 'List',
  components: { Pagination, DialogDebtCorrection, DialogEditStatistic, DialogImportExcel, exportVoucher },
  data() {
    return {
      tableData: [],
      loadingTable: false,
  ...
  watch: {
    dateRange(value) {
      if (value) {
        this.listQuery.start_date = moment(value[0]).add(7, 'hours').unix()
        this.listQuery.end_date = moment(value[1]).add(7, 'hours').unix()
      } else {
    ...
    dateRangePayment(value) {
      ...
      if (value) {
        this.listQuery.start_date_payment = moment(value[0]).add(7, 'hours').unix()
        this.listQuery.end_date_payment = moment(value[1]).add(7, 'hours').unix()
      } else {
  ...
  methods: {
    ...
    exportVoucher(value) {
      this.objectVoucher = value
      this.showVoucher = true
    },
    downloadSampleFile() {
      const params = ['STT', 'ma_bill', 'san_pham_ngoai', 'ngay_thanh_toan', 'vID_dai_ly', 'ngay_dai_ly_thanh_toan']
      const data = [['1', 'GB111111', 'OUT111,OUT122', '01-09-2023', '101253', '01-09-2023']]
      const baseUrl = process.env.VUE_APP_BASE_API
      const url = `${baseUrl}/public/file-excel-example?headers=${JSON.stringify(params)}&data=${JSON.stringify(data)}`
      statisticResource.downloadFile({ headers: JSON.stringify(params), data: JSON.stringify(data) })
        .then(res => {
          console.log('res..', res)
    ...
    onSearchAgency() {
      searchResource.getAllAgency().then((res) => {
        if (res.error_code === 0) {
          this.AgencyList = res.data.filter(i => i.name)
          this.AgencyList.unshift({ id: -1, name: 'Khách lẻ' })
    ...
    onGetList() {
      ...
      statisticResource.list(params).then(res => {
        ...
        // lặp qua mảng để thêm thêm trường prodcut_name
        this.tableData.forEach((item) => {
          item.product_name = item.product?.name
          item.billOutsidePayment.forEach((bill) => {
            bill.product_info = JSON.parse(bill.product_info)
          })
        })
    ...
    checkDayDebt(item) {
      if (moment(item['day_debt']).isBefore(moment()) && item['type_purchase'] === 0) {
        return true
      }
      return false
    },
    onChangeAgency(value) {
      if (!isEmpty(value)) {
        const agency = this.AgencyList.find(item => item.id === value)
        this.listQuery.agency_id = agency.id
        this.listQuery.agency_name = agency.name
      } else {
        delete this.listQuery.agency_id
        delete this.listQuery.agency_name
      }

      if (value === -1) {
        this.listQuery.agency_id = -1
        this.listQuery.agency_name = -1
      }
