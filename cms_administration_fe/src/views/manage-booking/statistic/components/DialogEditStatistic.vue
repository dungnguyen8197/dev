<template>
  <div>
    <el-dialog v-loading="loadingDialog" :visible.sync="showDialogEdit" :before-close="handleClose" width="95%">
      <el-row>
        ...
        <el-form ref="ruleForm" :model="dataRequest" :rules="rules">
          ...
          <el-row class="row-price-statistic">
            ...
            <el-col class="col-top-statistic" :span="5">
              <div class="grid-content bg-purple-dark">
                <el-form-item label="Hạng mục" prop="categories">
          ...
          <table class="table-golfer border table-bordered w-100">
            <thead>
              <tr>
                <th class="p-10 text-center word-break" style="width: 320px;">Golfer</th>
                <th class="p-10 text-center word-break" style="width: 150px">Số lượng khách</th>
                <th class="p-10 text-center word-break">Khách lẻ / Đại lý</th>
                <th class="p-10 text-center word-break">Ngày thanh toán</th>
                <th class="p-10 text-center word-break">Ngày đại lý thanh toán</th>
                <th class="p-10 text-center word-break">Người thanh toán</th>
                <th class="p-10 text-center word-break">Phụ phí</th>
                <th class="p-10 text-center word-break">Doanh thu phụ phí</th>
                <th class="p-10 text-center word-break">Mã giảm giá</th>
                <th class="p-10 text-center word-break">Refund cho Golfer</th>
              </tr>
            </thead>
            <tbody v-if="dataRequest.listUser.length > 0">
              <tr v-for="(booked, index) in detailStatistic.booked" :key="index">
                <td class="p-10">
                  <div class="info-custom">
                    <div style="width: 90px">
                      ...
                      <el-input v-model="dataRequest.listUser[index].name_out_system" hidden="hidden">
                        {{ booked.name_out_system }}
                      </el-input>
                    </div>
                    <p>{{ formatNumber(booked.price_booking) }}đ</p>
                    <div style="width: 150px">
                      <el-select v-model="dataRequest.listUser[index].type_purchase" disabled class="w-100" size="small">
                        <el-option
                ...
                <td class="p-10">
                  <div class="mb-1">
                    <el-input v-model="dataRequest.listUser[index].quantity_guest" disabled placeholder="Số lượng khách" size="small" type="number" />
                  </div>
                  <div>
                    <el-input v-model="dataRequest.listUser[index].passport" disabled placeholder="Passport/CCCD" size="small" />
                  </div>
                </td>
                <td class="p-10">
                  <el-select
                  ...
                  >
                    <el-option
                      v-for="agency in agencies.filter(i => i.name)"
                      :key="agency.id"
                      :label="agency.name"
                      :value="agency.name"
                    />
                ...
                <td class="p-10">
                  <el-select v-model="dataRequest.listUser[index].is_refund" disabled size="small">
          ...
          <el-form-item>
            ...
            <el-button size="mini" type="success" :loading="loadingSubmit" @click="handleUpdate">Xác nhận thanh toán refund (kế toán)</el-button>
          </el-form-item>
        </el-form>
      </el-row>
    </el-dialog>
...
export default {
  ...
  watch: {
    statisticId(id) {
      console.log('statisticId__________', id)
    },
    objectData(val) {
      if (val) {
        this.requestDetailStatistic(val.id)
      }
    }
  },
  created() {
  ...
  methods: {
    handleUpdate() {
      ...
      const body = {
        ...
        passport: this.dataRequest.listUser.map(d => d.passport),
        name_out_system: this.dataRequest.listUser.map(d => d.name_out_system),
        agency: this.dataRequest.listUser.map(d => ({ agency_name: d.agency, id_agency: d.id_agency || null })),
    ...
    //   searchResource.searchAgency().then(res => {
    ...
    async requestDetailStatistic(id) {
      ...
      statisticResource.detailStatistic(id).then(res => {
        console.log(' this.agencies________111111111111111111', this.agencies)

        console.log('res_____________', res)
        const { error_code, data } = res
        if (error_code === 0) {
          this.listPriceByAgent = data.list_price_tee
          this.detailStatistic = data.pax
          const supplierByRegion = data.supplierByRegion
          console.log('supplierByRegion__________', supplierByRegion)
          const user = []
          this.detailStatistic.booked.map((booked) => {
            console.log('booked__________', booked, data.pax.revenue_price)

            // let priceByAgency = 0
            // priceByAgency -= data.pax.revenue_price
            // console.log('pass qua đây', priceByAgency)

            // if (booked.agency_name) {
            //   console.log(' this.agencies________111111111111111111', this.agencies)
            //   const agency = this.agencies.find(i => i.name === booked.agency_name)
            //   console.log('agency______________', agency)
            //   const normal_price = data.list_price_tee?.find(i => i.agency_id === agency?.id && i.tee_time_id === data.pax.tee_time_id)?.normal_price
            //   if (normal_price) {
            //     priceByAgency += normal_price
            //   } else {
            //     priceByAgency = null
            //   }
            // } else {
            //   priceByAgency = null
            // }
            // console.log('priceByAgency______', priceByAgency)
            const surcharge_revenue = booked.surcharge_revenue
            console.log('booked..................................................', booked)
            const dataUser = {
              pax_booked_id: booked.id,
              vga: booked.user_id,
              agency: booked.agency_name,
              id_agency: booked.id_agency,
          ...
          console.log(' this.dataRequest.listUser____________', this.dataRequest.listUser)
          ...
          this.dataRequest.supplier = this.detailStatistic.supplier_id || supplierByRegion.id
          ...
          this.dataRequest.tee_time_id = this.detailStatistic.tee_time_id
          this.categories = data.list_category
          if (this.detailStatistic.supplier || !isEmpty(supplierByRegion)) {
            this.suppliers = this.detailStatistic.supplier ? [this.detailStatistic.supplier] : [supplierByRegion]
          } else {
            this.suppliers = []
          }
          console.log(' this.suppliers___________', this.suppliers)
        }
      }).finally(() => {
        this.loadingDialog = false
      })
    },
    handleAddPriceSurcharge(value, index) {
      if (value) {
        const agency_id = this.agencies?.find(i => i.name === value)?.id
        const price = this.listPriceByAgent?.find(i => i.agency_id === agency_id && i.tee_time_id === this.dataRequest.tee_time_id)?.normal_price
        if (price) {
          this.dataRequest.listUser[index].surcharge_revenue = price - this.dataRequest.revenue_price
        } else {
          this.dataRequest.listUser[index].surcharge_revenue = null
        }
      } else {
        this.dataRequest.listUser[index].surcharge_revenue = null
      }
