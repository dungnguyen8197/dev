<template>
  <el-dialog :visible.sync="showDialog" :before-close="handleClose" width="70%" title="Thông tin nhà cung cấp1">
    <table style="width: 100%; text-align: center" class="table-golfer border  w-100">
      <thead>
        <th style="width: 50px;">#</th>
        <th style="min-width: 180px;">Tên dịch vụ</th>
        <th style="min-width: 180px;">Mã hàng</th>
        <th style="min-width: 180px;">Nhà cung cấp</th>
        <th style="min-width: 180px;">Giá nhập</th>
        <th style="min-width: 180px;">Xác nhận TT NCC</th>
      </thead>
      <tbody>
        <tr v-for="(item, index) in list_service" :key="index">
          ...
          <td v-if="data_pax && data_pax.is_purchase_supplier == 0 && !comfirm_done">
