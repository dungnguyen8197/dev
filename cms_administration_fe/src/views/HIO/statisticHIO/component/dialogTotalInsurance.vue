      ...
      <el-col>
        ...
        >
          ...
          <el-table-column
            ...
            label="Tên giải đấu"
          />
          <el-table-column prop="title" label="Tên Sân" />
...
export default {
  ...
  methods: {
    totalInsurance() {
      const query = {
        agent_id: this.objectTotalInsurance.id,
      ...
      console.log('this', this.objectQuery)
      typeServiceResource.listTotalInsuranceByAgent(query).then(res => {
        console.log(res)
    ...
    async handleDownload() {
      ...
      console.log('ádsadasdwewqeqwgf', query)
      let list_all = []
      this.loadingExportExcel = true
      while (!stop) {
        try {
          const res = await typeServiceResource.listTotalInsuranceByAgent(query)
    ...
    formatJson(filterVal, jsonData) {
      return jsonData.map(v => filterVal.map(j => {
        if (j === 'name') {
          ...
          return +v[j] === 1 ? 'Đã trả' : +v[j] === 2 ? ' Ghi nợ' : +v[j] === 3 ? ' Công nợ' : 'Không ghi nợ'
        } else if (j === 'time_pay') {
          return v[j]
        } else if (j === 'name') {
          return v[j]
