<template>
  <el-dialog title="Tạo Booking Tee Time" :visible.sync="showDialog" width="95%" :before-close="handleClose">
    <div
    ...
    >
      <el-row class="p-10 border-1 pb-10 mb-10">
        <el-row class="dialog-add-golfer pb-10">
          ...
          <el-col class="p-10" :xs="24" :sm="12" :md="8" :lg="4" :xl="4">
            ...
            <br>
            <el-select v-model="postForm.host_user_id" clearable placeholder="Người nhận hoàn tiền">
              <el-option v-for="item in listGolfers" :key="item.value" :label="item.label" :value="item.value" />
            </el-select>
          </el-col>
          <el-col class="p-10" :xs="24" :sm="12" :md="8" :lg="4" :xl="4">
            <label>Loại hố</label>
            <br>
            <el-select v-model="postForm.type_hole" placeholder="Loại hố">
          ...
          <el-col class="p-10">
            <el-button type="primary" size="mini" @click="searchTeeTime">Tìm Tee Time</el-button>
          </el-col>
          <el-col class="p-10" :xs="24" :sm="12" :md="8" :lg="4" :xl="4">
            <label>Teetime</label>
            <br>
            ...
            >
              ...
              >
                ...
                <i v-if="item.is_lock == 1" class="el-icon-lock" />
        ...
        <el-row class="mb-10">
          <el-col :xs="12" :sm="12" :md="12" :lg="12" :xl="12">
            <!--                    <label>Sân: </label> <span>{{ facilityModel.label }}</span>-->
            <!--                    <br>-->
            <!--                    <label>Ngày: </label> <span>{{ (postForm.dayPlayed) }}</span>-->
            <!--                    <br>-->
            <!--                    <label>Đường: </label> <span>Đường A</span>-->
            <!--                    <br>-->
            <!--                    <label>Giờ chơi: </label> <span>07:00</span>-->
            <!--                    <br>-->
            <!--                        <label>Giá: </label>-->
            <!--                        <span v-if="postForm.pax !== null"> {{ formatNumber(postForm.pax.price_18_hole) }} đ/pax</span>-->
            <!--                    <br>-->
            <!--                    <label>Số người chơi: </label> <span>{{ listGolfers.length }}</span>-->
            <!--                    <br>-->
            <!--                    <label>Người nhận hoàn tiền: </label> <span>{{ postForm.host_user_id }}</span>-->
            <!--                    <br>-->
            <!--                    <label>Loại đơn: </label> <span>{{ postForm.order_type }}</span>-->
            <!--                    <br>-->
            <!--                    <label>Thành tiền: </label> <span>{{ postForm.total_price }}</span>-->
          </el-col>
          <el-col :xs="12" :sm="12" :md="12" :lg="12" :xl="12">
            <!--                    <el-upload-->
            <!--                        class="upload-demo"-->
            ...
            <!--                        <el-button style="margin-left: 10px;" size="small" type="success">upload to server-->
            <!--                        </el-button>-->
            <!--                        <div class="el-upload__tip" slot="tip">xlsx files import golfer</div>-->
            <!--                    </el-upload>-->
          </el-col>
        </el-row>
      </el-row>
            ...
            <tbody>
              <tr v-for="(golfer, key) in listGolfers" :key="key">
                <th style="text-align: left" class="col-3">
                  {{ golfer.user_id == -1 ? `(OUT) ` + golfer.name_out_system : golfer.name_out_system }}
                </th>
                <td>
                ...
                <td>
                  <el-select
                    v-model="listGolfers[key].surcharge_id"
                    clearable
                    multiple
                    filterable
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" :loading="loadingSubmit" @click="onSubmitAddGolfer">Thêm mới</el-button>
      </span>
...
const listService = [
  {
    value: 1, label: 'CUP', type: 'shop', commodity_codes: [
      { value: 1, label: 'CUP 1' },
      { value: 2, label: 'CUP 2' },
      { value: 3, label: 'CUP 3' },
      { value: 4, label: 'CUP 4' }
...
export default {
  ...
  watch: {
    ...
    objectDataProduct(value) {
      ...
      service.product_info = value.product
  ...
  methods: {
    ...
    onSubmitAddGolfer() {
      if (this.listGolfers.length === 0) {
        this.$message.warning('Bạn cần phải chọn người chơi!')
        return
      }
      if (this.isBooking === -1) {
        this.$message.warning('Bạn cần phải chọn sân đánh!')
        return
      }
    ...
    onSearchInsurrance(id) {
      ...
      bookingPaxResource.searchInsurance(query).then(res => {
        if (res.error_code === 0) {
          this.listHoldInOne = res?.data?.data?.data
          console.log(' this.listHoldInOne.............................', this.listHoldInOne)
        }
      })
    },
    getListAgent() {
    ...
    searchTeeTime() {
      ...
      console.log('searchTeeTime', this.postForm.dayPlayed)
    ...
    getListInsurrance() {
      priceByHoleInsurrance.lists().then(res => {
        if (res.error_code === 0) {
          const result = res.data
          if (result.length > 0) {
            this.listInsurrance = result.map(value => {
              ...
              if (nonExistingUserIds.length > 0) {
                ...
                for (let i = 0; i < rowObject.length; i++) {
                  ...
                  if (row['user_id'] !== -1) {
                    ...
                    row['name_out_system'] = `VID${row['user_id']} - ${user.fullname}`
                    delete row['phone']
                  }
                  delete row['fullName']
                }
    ...
    handlePriceByGolfer(key) {
      ...
      if (golfer.insurance_pax) {
        const { price } = this.listInsurrance.find(item => item.value === golfer.insurance_pax)
        golfer.price += price
      }
      if (golfer['surcharge_id'] && golfer['surcharge_id'].length > 0) {
        console.log(golfer.surcharge_id)
        this.listSurcharge.filter(item => golfer['surcharge_id'].includes(item.id))
          .map(item => {
    ...
    onSearchOtherService(value) {
      ...
      bookingPaxResource.searchOtherService(query).then(res => {
        if (res.error === 0) {
          this.listOtherService = res?.list
        } else {
    ...
    changeHoldInOne(key) {
      const service = this.postForm.service[key]
      const hio = this.listHoldInOne.find(item => item.id === service.hio_id)
      service.price = hio.total_pay
      service.quantity = 1
      service.hio_info = JSON.stringify(hio)
    },
