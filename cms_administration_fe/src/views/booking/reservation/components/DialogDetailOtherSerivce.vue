...
export default {
  ...
  methods: {
      ...
      bookingPaxResource.detailProduct({ id: id }).then(res => {
        if (res.error === 0) {
          ...
          if (data.name_param_first) {
            data.size_new = data[data.name_param_first.toLowerCase()].split(',')
          }
    ...
    onSubmitAddProduct() {
      if (this.objectData.name_param_first && isEmpty(this.bodyRequest.size)) {
        this.$message.warning('Bạn cần phải chọn size!')
        return
      }
      if (this.objectData.name_param_second && isEmpty(this.bodyRequest.color)) {
        this.$message.warning('Bạn cần phải chọn màu!')
        return
      }
