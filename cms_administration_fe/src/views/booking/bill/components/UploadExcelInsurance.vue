    ...
    readerData(rawFile) {
      ...
      return new Promise((resolve, reject) => {
        ...
        reader.onload = e => {
          ...
          const arrayVgaError = []
          console.log(results)
          bookingPaxResource.onCheckGolferId({ list: results }).then(response => {
            console.log(response)
            if (response.error_code === 0) {
              if (response.data.data.list_user.length > 0) {
                response.data.data.list_user.map(row => {
              ...
              if (response.data.data.list_not_found.length > 0) {
                response.data.data.list_not_found.map(row => {
                  const vga = row['MÃ vID']
                  const fullname = row['HỌ VÀ TÊN ']
                  const check = this.listsGolferError.filter(item => parseInt(item.value) === parseInt(vga))
                  if (vga && check.length === 0) {
                    const item = { golfer: { system_avatar_path: 'https://livenet.vhandicap.com/images/av.png', id: vga, fullname: `${fullname}` }, label: `vID${vga}`, value: parseInt(vga) }
                    arrayVgaError.push(item)
                  }
                })
              }
