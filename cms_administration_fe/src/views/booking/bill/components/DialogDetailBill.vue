...
import { listOrderType, listHIO, listService, listTypeHole, LIST_TYPE_CARD_SCB, CUSTOMER_TYPE } from '@/utils/defaultOptions'
      ...
      if (res.error_code === 0) {
        ...
        if (data.pax) {
          ...
          if (!isEmpty(other_service)) {
            ...
            this.facilityModel = this.facilities[0]
            this.dayPlayedModel = other_service?.bookingOutside?.date_played
          }
        }
        const process = []
        ...
        for (let i = 0; i < data.golfers.length; i++) {
          ...
          if (booked?.card_puid) {
            ...
            if (indexPuid !== null && +indexPuid !== -1) {
              this.listGolfers[indexPuid].code_card = card?.Code
              ...
              data.golfers[indexPuid].code_card = card?.Code
            }
            console.log('indexPuid_______________', indexPuid)
          ...
          if (!isEmpty(golfer.agency_id)) {
            console.log('vào dâyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy')
            this.handleAgency(i)
          }
        }
