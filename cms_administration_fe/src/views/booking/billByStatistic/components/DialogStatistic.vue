        ...
        <el-row>
          ...
          <el-form ref="ruleForm" :model="dataRequest" :rules="rules">
                      ...
                      <div class="mb-10">
                        <div class="info-custom">
                          ...
                          <div style="width: 170px">
                            <el-select v-model="booked.type_purchase" :disabled="booked.is_cancel > 0 || Boolean(booked.is_deleted)" class="w-100" size="small" @change="checkType(index)">
                              <el-option
                                v-for="(arr, i) in arrayTypePurchase.filter(item => item.value ==14 || item.value == clone_listGolfers[index].booked.type_purchase)"
                                :key="i"
                                :label="arr.label"
                                :value="arr.value"
                              />
            ...
            <el-row v-if="dataRequest?.pax" class="row-price-statistic mb-20">
              <el-col class="col-top-statistic" :span="11">
                      ...
                      <template v-for="(service, key_service) in postForm.service">
                        <tr :key="key_service">
                          ...
                          <td class="text-center">
                            <el-select v-model="postForm.service[key_service].type_purchase" :disabled="postForm.service[key_service].is_cancel > 0" size="small" style="width: 100%" clearable placeholder="Hình thức thanh toán" @change="checkTypeServiceChild(key_service)">
                              <el-option
                                v-for="(arr, i) in convertListTypePurchaseChild(key_service)"
                                :key="i"
                                :label="arr.label"
                                :value="arr.value"
                              />
            ...
            <el-row class="row-price-statistic mb-20">
              <el-col class="col-top-statistic" :span="11">
                <label>Lưu ý của khách hàng (nếu có)</label>
                <el-input
              ...
              <el-col class="col-top-statistic" :span="2" />
              <el-col class="col-top-statistic" :span="11">
                <label>Note kế toán</label>
                <el-input
                  v-model="postForm.note_accountant"
                  placeholder="Note kế toán"
                  type="textarea"
                  :rows="5"
                />
              </el-col>
...
export default {
  ...
  methods: {
    async onSubmitForm() {
      // if (isEmpty(this.listGolfers.filter(item => item.type_purchase !== 14 && item.is_cancel !== 1 && !item.is_deleted).find(item => +item.user_id !== -1))) {
    ...
    handleUpdate() {
      ...
      const data = {
        ...
        list_hio: this.dataRequest.list_hio.map(item => ({ ...item, html_cert_insurrance: '', list_user_insurrance: item.list_user_insurrance.map(t => ({ ...t, Users: {}})) })),
      ...
      const compressedData = pako.deflate(JSON.stringify({
        ...
        service_old: this?.clone_convert_service?.map(item => ({ id: item.id, type_purchase: item.type_purchase })),
        note_accountant: this.postForm?.note_accountant
      }), { to: 'string' })
      const compressedString = btoa(encodeURIComponent(compressedData))
      bookingPaxResource.updateStatisticBill({ data_zip: compressedString }).then(async(res) => {
        if (+res.error_code === 0) {
    ...
    async onGetDetail(id) {
      ...
      bookingPaxResource.detailBill({ bill_id: id, all_service: true }).then(async(res) => {
        if (res.error_code === 0) {
          ...
          for (let i = 0; i < data?.golfers?.length; i++) {
            ...
            const addGolfer = {
              ...
              surcharge_facility: golfer?.surcharge_facility_id || []
        ...
        data.list_hio.filter(item => list_hio_dif.includes(item.id) && +item.is_outing === 0 && +item.is_send_pvi === 1 && +item.options_money_id !== 9 && moment(item?.created_at, 'HH:mm:ss,DD-MM-YYYY') < date_not_free_d20) // && +item.options_money_id !== 9 ----> co tang D20
          .map(item1 => ({
            User: item1.User,
          ...
          const convert_service_data = data.service.map(item => ({
            ...item,
            ...
            host_user_id: item.type === 'other_service' ? item.bookingOutside.user_id_pay : (item.type === 'shop' ? item?.user_id ? item?.user_id : '' : '')
          }))
          ...
          this.postForm = {
            ...
            note: data.note,
            note_accountant: data.note_accountant
          }
  ...
  : insurance_provider?.find(item2 => +item2.options_money_id === +item.value)?.is_customer_pay === 1 ? '(Đã thanh toán)' : '(Chưa thanh toán)'}`)
    ...
    convertListTypePurchaseChild(index) {
      const service = this.postForm.service[index]
      const service_old = this.clone_convert_service[index]
      if (+service.id === -1) {
        return this.listTypePurchaseChildService.filter(item => +item.value === 14 || +item.value === +service_old.type_purchase)
      } else if (+service.id === 2) {
        if (+service_old.type_purchase === 1) {
          return [
        ...
        const clone_array = [...this.listTypePurchaseChildService.filter(item => +item.value === 14 || +item.value === +service_old.type_purchase)]
